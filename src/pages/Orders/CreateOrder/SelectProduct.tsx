...
const SelectProduct = () => {
  ...
  // Get selected products from the store
  const selectedProducts = useSelectedOrderProducts(
    state => state.selectedItems
  );
  const addSelectedItems = useSelectedOrderProducts(
  ...
  const handleItemSelect = (item: PRODUCT_TYPE) => {
  ...
  const handleAddToStore = () => {
    // Replace the store contents with our local selection
    addSelectedItems(localSelectedItems);
    setOpen(false);
  };
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className=" w-full h-screen px-0 sm:max-w-[36.5rem] sm:h-auto sm:rounded-lg ">
        ...
        <SheetDescription className="px-5 pt-5">
          ...
          <ScrollArea className="h-[calc(100vh-18rem)] max-lg:h-[30rem] pb-10 max-lg:pb-12 mt-6 max-lg:mb-1">
            {isLoading && (
            ...
            <div className="flex flex-col gap-4">
              {(searchResult.length > 0 ? searchResult : products).map(
                (item: PRODUCT_TYPE, index: number) => {
          ...
          <div className="absolute max-lg:fixed max-lg:bg-white bottom-0 right-0 px-5 pb-5 max-lg:py-5 w-full">
            <Button
              className="w-full"
